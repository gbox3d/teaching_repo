...
class ControlActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.ledSwitch.setOnCheckedChangeListener { _, isChecked ->
            // TODO(2) LED 번호 토글 — send로 명령 보내기(15주차 1일차 리허설).
            //         pinEdit의 글자가 비어 있으면 Toast "LED 번호를 입력하세요".
            //         숫자로 바꾼 번호가 12번 isAllowedIndex()를 통과하지 못하면 Toast "허용되지 않는 번호"를 띄우고 보내지 않는다.
            //         통과하면 isChecked가 true일 때 "on 번호", false일 때 "off 번호"를 Bleuno.client?.send로 보내고,
            //         보낸 명령을 logText에 한 줄 append한 뒤 15번 pauseButtons()를 부른다. 7번 [전체 끄기]가 같은 모양이다.
        }
    ...
    override fun onStart() {
        ...
        Bleuno.client?.onMessage { json ->
            ...
            if (event == "input") {
                ...
                // TODO(2) 응답 표시(15주차 1일차 리허설).
                //         logText에 "응답: 받은 JSON" 한 줄을 append한다.
                //         result가 "ok"가 아니면(err·fail) 로그 글자를 colors.xml의 log_error 색으로 바꾸고, BleunoMessage.message(json)을 AlertDialog로 알린다.
                //         "ok"면 로그 글자를 log_ok 색으로 되돌린다. 색은 ContextCompat.getColor(this, R.color.색이름)으로 꺼낸다.
            }
        }
    }
    ...
    private fun isAllowedIndex(index: Int): Boolean {
        // TODO(2) 보내도 되는 번호인지 확인하기(15주차 1일차 리허설).
        //         index가 0·1·2·3 가운데 하나이면(listOf(…).contains(…)) true를 돌려준다. -1(전체)이어도 true를 돌려준다.
        //         아래 return false는 "그 밖의 번호"일 때 돌려주는 줄이다. 지우지 말고 그 위에 채운다(지우면 빌드가 안 된다).
        return false
    }
