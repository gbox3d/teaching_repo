...
## 1일차 · 8–20분 ③ — 에뮬레이터에서 화면 돌리기

| 방법 | 하는 법 |
|---|---|
| 버튼 | 에뮬레이터 **위쪽** 도구 막대의 회전 버튼 **Rotate Left** · **Rotate Right** |
| 단축키 | 에뮬레이터 화면을 한 번 누른 뒤 **Ctrl+L** · **Ctrl+R** (맥 ⌘+L · ⌘+R) |

- 누를 때마다 기기가 90도씩 돕니다. 반대쪽 버튼으로 세로로 되돌립니다.
- 에뮬레이터가 별도 창이면 도구 막대는 **오른쪽**, 단축키는 **Ctrl+←** · **Ctrl+→** 입니다.

**기기만 돌고 앱 화면이 그대로면** 기기의 자동 회전이 꺼진 것입니다.

1. 화면 구석에 몇 초 동안 뜨는 **회전 아이콘**을 누릅니다. 또는
2. 화면 위쪽을 아래로 끌어 빠른 설정에서 **자동 회전**(Auto-rotate)을 켭니다.
