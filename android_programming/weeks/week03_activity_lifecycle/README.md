...
|---|---|---|---|
| 1일차 | Logcat 창과 태그 필터, `Log.d`, 생명주기 콜백 7개, 에뮬레이터에서 화면 돌리는 법, 회전하면 화면이 새로 만들어지는 그림, `Toast` | 2주차 프로젝트에 콜백 7개와 `Log.d` 추가 → 실행·홈·복귀·회전·뒤로 순서를 기록표에 적기 → `+1`에 Toast | 콜백 순서 기록표와 Logcat |
