...
아래 "실행 결과" 표와 멈춤 증상은 에뮬레이터 실행 확인(QA) 전에 코드와 제공 라이브러리 동작으로 판단한 **예상**이다. 오류 문구(`Unresolved reference …` 등)는 빌드로 확인한 원문이다.
...
| 받는 중 회전 | [온습도 받기 시작] 켜짐, [중지] 꺼짐 | 빈 목록으로 다시 시작 |
...
| (확인용) 51번 `delay(10000)`을 잠시 `delay(1000)`으로 바꾸고 목록 줄 누르기 | 약 1초 뒤 Toast `연결 시간이 초과되었습니다`, 상태 `연결 시간 초과 — [다시 시도]를 누르세요`, [검색] 켜짐, [다시 시도] 보임. 확인한 뒤 되돌린다 |
