...
14주차는 주차별 실습 점수 대상이 아니다. 이번 주 결과물은 **2차 과제(20점 = 발표 12 + 레포트 8)** 이며, 최종 앱의 캡처 세트와 2~3분 시연(권한 안내 → 장치 목록 → `준비됨` → LED on/off → 온습도 이력 2줄 이상 또는 입력 이벤트 2줄 → `끊김` + [재연결])이다.
...
- [ ] [온습도 받기 시작]으로 입력 이력에 `시:분:초 [온도,습도]` 줄이 2줄 이상(시각이 약 3초 간격인 줄) 쌓이고 [중지]로 멈춘다(Fake면 입력 이벤트 `입력=1`/`입력=0` 2줄도 된다).
- [ ] `끊김`이 되면 Toast와 [재연결]이 보이고, [재연결]로 `준비됨`까지 간다. 제어 화면에서 끊겨도 안내가 보인다.
- [ ] 연결을 시작하고 10초 안에 `준비됨`이 아니면 `연결 시간이 초과되었습니다`와 [다시 시도]가 보인다.
- [ ] 레포트에 넣을 캡처(권한·목록·준비됨·LED·입력 2줄·끊김+[재연결]·시간 초과)와 2~3분 시연 절차를 준비했다.
