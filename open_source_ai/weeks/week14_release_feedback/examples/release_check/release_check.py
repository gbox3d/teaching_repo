"""릴리스 준비 점검 도구 (읽기 전용).
저장소 폴더의 릴리스 문서 세트·버전·비밀 관리 상태를 PASS/WARN/FAIL/INFO 로 보여 주고
outputs/release-check-<시각>.json 에 남긴다. 어떤 파일도 수정하지 않으며 모델·GPU·네트워크가 필요 없다."""
...
REQUIRED_FILES = (
    ...
    ("SOURCES.md", "fail", "모델·데이터·코드 출처와 라이선스 표가 필요하다"),
    (".env.example", "fail", "설정 항목을 보여 줄 견본이 필요하다"),
    ("uv.lock", "warn", "없으면 남의 PC 에서 uv sync --frozen 이 불가능하다"),
)
ENV_TRACKED_MESSAGES = {
    None: ("warn", "git 으로 .env 추적 여부를 확인하지 못했다"),
    True: ("fail", ".env 가 git 에 추적되고 있다 - 추적을 끊고 토큰을 회전한다"),
    False: ("pass", ".env 는 git 에 추적되지 않는다"),
}
...
def check_file(repo: Path, name: str, level_if_missing: str, why: str) -> Item:
    exists = (repo / name).is_file()
    return item(name.lower(), "pass" if exists else level_if_missing, f"{name} 있음" if exists else f"{name} 없음 - {why}")
...
def check_readme(repo: Path) -> list[Item]:
    ...
    # 도구는 '#' 로 시작하는 제목 줄만 본다. 본문에 내용이 있어도 제목이 없으면 찾지 못한다.
    heads = [ln.lstrip("#").strip().lower() for ln in text.splitlines() if ln.startswith("#")]
    items = [item("readme", "pass", f"README.md 제목 {len(heads)}개")]
    missing = [name for name, hints in README_SECTIONS.items()
               if not any(hint in head for head in heads for hint in hints)]
    items.append(item("readme.sections", "fail" if missing else "pass",
                      "빠진 절: " + ", ".join(missing) if missing else "8개 절이 모두 있다"))
    has_ai = any(re.search(r"\bai\b", h) or "인공지능" in h for h in heads) or (repo / "AI_USAGE.md").is_file()
    items.append(item("readme.ai_usage", "pass" if has_ai else "warn",
                      "AI 도구 사용 내역 표기가 있다" if has_ai else "AI 도구 사용 내역 절(README) 또는 AI_USAGE.md 가 없다"))
    return items


def check_license(repo: Path) -> Item:
    head = read_text(repo / "LICENSE")[:400].lower()
    if not head:
        return item("license", "fail", "LICENSE 없음 - 라이선스 없는 코드는 남이 쓸 수 없다")
    kind = next((h for h in LICENSE_HINTS if h.lower() in head), None)
    return item("license", "pass" if kind else "warn",
                f"LICENSE 있음 ({kind} 계열로 보임)" if kind else "LICENSE 는 있지만 첫 부분에서 라이선스 이름을 찾지 못했다")


def check_changelog(repo: Path) -> Item:
...
def check_citation(repo: Path) -> Item:
    ...
    return item("citation", "fail" if missing else "pass",
                "CITATION.cff 필드 부족: " + ", ".join(missing) if missing else "CITATION.cff 필수 필드 있음")


def git_tracked(repo: Path, name: str) -> bool | None:
    """git 이 name 을 추적하면 True, 아니면 False, 확인 불가면 None."""
    try:
        out = subprocess.run(["git", "-C", str(repo), "ls-files", "--", name],
...
def check_secrets(repo: Path) -> list[Item]:
    items = [item("env.tracked", *ENV_TRACKED_MESSAGES[git_tracked(repo, ".env")])]
...
def check_version(repo: Path, tag: str | None) -> list[Item]:
    match = re.search(r'^version\s*=\s*"([^"]+)"', read_text(repo / "pyproject.toml"), re.M)
    if not match:
...
def run_checks(repo: Path, tag: str | None, require_model_card: bool) -> list[Item]:
    ...
    items += [check_license(repo), check_changelog(repo), check_citation(repo)]
    items += [check_file(repo, name, level, why) for name, level, why in REQUIRED_FILES]
    items.append(check_file(repo, "MODEL_CARD.md", "fail" if require_model_card else "info",
                            "어댑터·모델을 공개하면 모델 카드가 필요하다"))
    return items + check_secrets(repo) + check_version(repo, tag)
...
def main() -> int:
    ...
    out_path = Path(args.out_dir) / f"release-check-{stamp}.json"
