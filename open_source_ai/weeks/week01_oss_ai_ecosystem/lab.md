...
개인 노트북에는 아무것도 깔려 있지 않은 것이 정상이다. 없는 것을 확인하고, 직접 설치하고, 설치되었음을 스스로 증명하는 것까지가 이번 블록이다.
...
이미 다섯 도구가 모두 정상인 PC는 설치 단계를 문제 2의 4·5번(설치 경로 확인)만 수행하고 계정 확인으로 넘어간다.
...
### 문제 2 · uv를 직접 설치하고 스스로 증명하기

`uv`는 3교시부터 학기 끝까지 모든 실습의 실행 도구다. 문제 1에서 `uv` 행이 `실패`였다면 여기서 직접 설치한다.

1. 설치 전에 예상을 적는다: **설치가 끝난 바로 그 창에서 `uv --version`을 치면 될 것 같은가? 그 이유는?**
2. 둘 중 하나로 설치한다. 관리자 권한은 필요 없다.
   ...
   ```powershell
   # 방법 2 · 학교·회사 정책으로 스크립트 실행이 막힌 PC
   winget install --id=astral-sh.uv -e
   ```

   설치 로그의 마지막 몇 줄을 읽는다. 어느 폴더에 무엇을 두었다고 적혀 있는지 확인한다.

3. **1번의 예상을 확인한다.** 설치한 창에서 그대로 `uv --version`을 실행해 본다. 결과를 적는다.
4. **PowerShell 창을 완전히 닫고 새 창을 연다.** 다시 실행한다.

   ```powershell
   uv --version
   ```

5. 어디에 설치되었는지 확인하고 점검표에 적는다. 설치 방법에 따라 경로가 다르다.

   ```powershell
   Get-Command uv | Select-Object -ExpandProperty Source
   ```

6. `env_check.ps1`을 다시 실행해 `uv` 행이 `실패`에서 `정상`으로 바뀌는지 본다.

   ```powershell
   .\env_check.ps1 -OutFile env_check_raw.md
   ```

7. 3번과 4번의 결과가 달랐다면 그 이유를 한 문장으로 적는다. 같았다면 왜 같았는지 적는다.

완료 조건:

- [ ] `uv --version`이 **새 창에서** 버전 문자열을 출력한다.
- [ ] `uv`의 설치 경로를 `Get-Command`로 확인해 적었다.
- [ ] 3번(설치한 창)과 4번(새 창)의 결과 차이 또는 무차이를 한 문장으로 설명했다.
- [ ] `env_check_raw.md`의 `uv` 행이 `정상`이다.

> **오늘 실습 시간에 설치하는 것은 `uv` 하나다.**
> `ollama`는 내려받을 것이 커서(바이너리만 4 GB, 모델은 별도) 실습 시간에 하지 않는다 — 아래 [숙제](#숙제--올라마-설치-4주차-전까지)를 본다. 점검표에는 `실패`와 "미설치 → 4주차 전까지 설치"라고 적는 것이 정답이다.
> `git`이 없다면 [설치 프로그램 목록](../../ta_setup_guide.md#필수-설치)의 Git for Windows 항목으로 설치하고 uv와 같은 순서(설치 → 새 창 → `Get-Command`)로 확인한다.
> `nvidia-smi`는 설치로 해결되지 않는다. GPU가 없는 노트북은 "GPU 없음 → CPU + 소형 모델" 대체 경로가 정답이다.
...
<details>
<summary>힌트 2 — uv 설치 스크립트가 실행되지 않는다</summary>

`irm ... | iex` 줄이 실행 정책이나 보안 정책에 막히는 PC가 있다. 정책을 바꾸려 하지 말고 winget으로 설치한다.

```powershell
winget install --id=astral-sh.uv -e
```

`winget`이라는 명령 자체가 없으면 Microsoft Store에서 "앱 설치 관리자(App Installer)"를 설치하거나, uv 공식 문서의 [Installation](https://docs.astral.sh/uv/getting-started/installation/) 페이지에서 Windows용 독립 실행 파일을 직접 내려받는다.

설치 방법이 달라지면 **설치 경로도 달라진다.** 문제 2의 5번을 반드시 수행한다.
</details>

<details>
<summary>힌트 3 — 설치했는데 새 창에서도 "명령을 찾을 수 없음"이 나온다</summary>

먼저 창을 정말 새로 열었는지 본다. VS Code 안의 터미널은 VS Code를 껐다 켜야 PATH가 갱신된다.

그래도 같으면 "설치가 안 된 것"과 "설치는 됐는데 PATH에 없는 것"을 구분한다. 순서대로 실행한다.

```powershell
Test-Path "$env:USERPROFILE\.local\bin\uv.exe"    # 공식 스크립트로 설치했을 때의 위치
$env:Path -split ';' | Select-String '\.local'    # PATH에 등록되었는지
& "$env:USERPROFILE\.local\bin\uv.exe" --version  # 전체 경로로 직접 실행
```

- 첫 줄이 `False`면 설치 자체가 실패했다. 설치 명령을 다시 실행하고 로그의 오류 줄을 읽는다.
- 첫 줄이 `True`인데 셋째 줄만 성공하면 PATH 등록 문제다. 이 상태를 "실패"로 적고 조치 칸에 근거(`uv.exe`는 있으나 PATH에 없음)를 적는 것이 정답이다.
- winget으로 설치했다면 경로가 다르다. `Get-Command uv` 또는 `winget list astral-sh.uv`로 확인한다.
</details>

<details>
<summary>힌트 4 — nvidia-smi는 되는데 VRAM 숫자가 이상하다</summary>
...
- 정상: 다섯 도구 모두 버전 문자열이 있고, GPU 표에 이름과 VRAM이 있다. `uv`는 설치 경로까지 적혀 있다.
- 경계 또는 실패: 도구 하나가 없거나 GPU가 없는 PC에서도 점검표를 끝까지 채웠고, 조치와 대체 경로가 적혀 있다.
- 설치: 개인 노트북에서 `uv`를 직접 설치했고, 설치한 창과 새 창의 결과 차이를 설명했다.
...
**실습실 PC**는 조교가 이미 설치했다. 점검표에 확인만 하면 된다.
**개인 노트북**은 4주차 수업 전까지 각자 설치해 온다. 4주차는 올라마가 켜져 있는 상태에서 시작한다.

오늘 실습 시간에 하지 않는 이유는 용량이다. 바이너리만 4 GB이고 모델은 그보다 크다. 한 강의실에서 동시에 내려받으면 수업이 멈춘다.
...
방법 3으로 <https://ollama.com/download/OllamaSetup.exe>를 받아 실행해도 된다.

> `Ollama.Ollama.Portable`은 **다른 패키지**다. 트레이 앱이 없는 압축 해제형이라 수업 절차와 맞지 않는다. 설치하지 않는다.

#### 확인

설치가 끝나면 올라마는 백그라운드에 상주하고, 로그인할 때마다 자동으로 뜬다.

```powershell
ollama --version
```

예상 결과는 셋 중 하나다. 어느 쪽인지 구분할 수 있어야 한다.

| 출력 | 뜻 | 할 일 |
|---|---|---|
| `ollama version is 0.x.y` 한 줄 | 설치·서버 모두 정상 | 없음 |
| `Warning: could not connect to a running Ollama instance`<br>`Warning: client version is 0.x.y` **두 줄** | 설치는 됐고 **서버만 꺼짐**. 이때 버전 줄은 나오지 않는다 | 작업 표시줄 트레이에서, 없으면 시작 메뉴에서 Ollama 실행 |
| `명령을 찾을 수 없음` | PATH 미갱신 또는 설치 실패 | PowerShell 창을 닫고 새로 연다. 방법 2·3은 새 창이 필요하다 |

#### 모델

**받을 모델 ID는 강의자가 공지한다. 공지 전에는 받지 않는다.** 참고로 교재 검증용 기본값의 크기는 아래와 같다(2026-09 기준, 태그가 재배포되면 바뀐다).
...
설치 위치와 모델 위치는 **각각 따로** 옮겨야 한다. 하나만 바꾸면 나머지는 C 드라이브에 남는다. 절차는 [설치 프로그램 목록 — 디스크가 부족한 PC](../../ta_setup_guide.md#디스크가-부족한-pc)에 있다.

완료 기준:

- [ ] 4주차 수업 전에 새 창에서 `ollama --version`이 버전 문자열을 출력한다.
- [ ] 강의자가 공지한 모델이 `ollama list`에 보인다.
- [ ] 1주차 점검표의 Ollama 행을 "미설치 → 설치함(날짜)"으로 갱신했다.
