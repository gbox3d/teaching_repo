...
| `torch_lab/.env.example` | `HF_EMBED_MODEL`, `HF_HOME`, `HF_HUB_OFFLINE`, `HF_TOKEN`(비워 둠) 예시. `.env`로 복사한다 | 3교시 |
...
5. `[6]` 절: `no_grad=False`일 때 추가 메모리가 층 수 × 배치 × 은닉 × 4바이트 근처로 늘어난다. 역전파에 쓸 활성화를 저장하기 때문이다. `no_grad=True`면 거의 0이다.
...
- `runlog.py`는 7주차 `embed.py`, 10주차 `train_lora.py`에서 그대로 가져다 쓴다. `RunLog(name, config, device)` → 실험 → `finish(metrics)` 순서만 지키면 된다.
...
| `HF_HOME` | (설정 시) | Hugging Face 캐시 위치. 실습실 공용 캐시를 쓸 때 지정 |
